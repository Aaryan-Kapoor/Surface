...
const COMMANDS: Record<string, CommandSpec> = {
  ...
  upgrade: {
    help: [
      ...
      "  under a non-default name. --progress-file writes each phase to a JSON file as it happens",
      "  (used by the PWA's one-click update, which reads it across the restart); --run-id stamps",
      "  those records so the server can tell this run's progress from anyone else's.",
    ].join("\n"),
    flags: { check: BOOL, json: BOOL, timeout: NUM, name: STR, "progress-file": STR, "run-id": STR },
