...
- `video/readme-banner/index.html` is the source of the README hero GIF, and
  `video/clips/*/index.html` are the four README showcase clips — all
  HyperFrames compositions (edit → `npm run render` in that directory →
  re-encode the GIF with the ffmpeg palette recipe in the PR that added
  them). The mp4 masters and old launch films (`video/archived/`) stay
  untracked.
