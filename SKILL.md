...
- **Codex CLI or Desktop:** run `surface codex setup` once. After that, ordinary `codex` and `codex resume NAME` commands attach automatically; management/non-interactive commands pass through, and `codex --no-surface` bypasses attachment once. Linux/macOS use the native Codex daemon. On Windows, Desktop remains separately opt-in through `surface codex launch`; normal Start-menu launches stay independent. Codex-created surfaces flow back into their exact live session without a waiter; dead-session wakes remain consent-gated and fail closed on approvals. Use `surface codex status` to diagnose the bridge; remove it with `surface codex setup --remove` after quitting Desktop. If setup is unavailable, fall back to a one-shot `surface wait`.
