...
- **Codex CLI or Desktop:** run `surface codex setup` once. Linux/macOS use the native Codex daemon. On Windows, quit Desktop and open the bridged instance with `surface codex launch`; the endpoint is process-scoped and normal Start-menu launches stay independent. Thereafter Codex-created surfaces flow back into their exact live session without a waiter; dead-session wakes remain consent-gated and fail closed on approvals. Use `surface codex status` to diagnose the bridge; remove it with `surface codex setup --remove` after quitting Desktop. If setup is unavailable, fall back to a one-shot `surface wait`.
