...
## Capture flow (`capture` / `runCdpCapture`, `server/thumbs.ts`)

1. Spawn Chrome headless with `--remote-debugging-port=0` against `about:blank` and a throwaway `--user-data-dir`. Flags include `--headless=new`, `--no-sandbox`, `--disable-dev-shm-usage`, and a long list of network/feature disables.
...
4. `Target.getTargets` → find the existing `page` target → `Target.attachToTarget` (`flatten:true`) to get a session. The capture attaches to the page Chrome already opened rather than creating a new one.
...
7. Wait a **fixed 6.5s** (`POST_NAVIGATE_DELAY_MS`, `server/thumbs.ts`) for animations/data to settle — there is no readiness signal; it is a constant delay.
...
- **SVG placeholder** (`renderThumbPlaceholder`, `server/render.ts`) is colored by mime via `paletteForMime` (HTML, VIDEO, AUDIO, PDF, MD, IMAGE, TEXT, FILE) with the wrapped title.
- `?regenerate=1` deletes the cached PNG, enqueues a fresh capture, and returns the placeholder immediately.
- **Cache busting:** the PWA requests `/artifacts/<id>/thumb?v=<updated_at>` so a new version refetches; on a `thumb_ready` event it swaps in `?v=Date.now()` (`client/app.js`). The `v` param is only a cache key — the route does not read it.
