...
const mcp = new Server(
  { name: "surface", version: "0.1.0" },
  {
    ...
    instructions: [
      ...
      "",
      "== WEBHOOK FAN-OUT ==",
      "Surface can optionally forward user actions to an external agent gateway. If SURFACE_WEBHOOK_URL and SURFACE_WEBHOOK_TOKEN are set, every surface action is POSTed as a structured JSON envelope `{ type: 'surface_action', surface_id, surface_title, action, data, created_at }` to `<SURFACE_WEBHOOK_URL><SURFACE_WEBHOOK_PATH>` (default path /hooks/agent). The receiving gateway can then call back into the Surface HTTP API to respond. OPENCLAW_GATEWAY_URL / OPENCLAW_HOOKS_TOKEN are accepted as legacy aliases.",
      "",
      "== OTHER TOOLS ==",
